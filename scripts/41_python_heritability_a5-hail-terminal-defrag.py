...
for idx, (cpg_col, cpg_pos) in enumerate(zip(selected_cpg_cols, selected_cpg_positions), start=1):
    ...
    # -----------------------------
    try:
        # Create a DataFrame with phenotype
        herit_df = pd.DataFrame({
            'phenotype': y
        })

        # Create a DataFrame for kinship matrix columns
        # Assign column names as K_0, K_1, ..., K_n
        k_columns = [f'K_{i}' for i in range(K_normalized.shape[0])]
        K_df = pd.DataFrame(K_normalized, columns=k_columns)
        print("Kinship matrix DataFrame created.")

        # Concatenate kinship DataFrame with herit_df
        herit_df = pd.concat([herit_df, K_df], axis=1)
        print("Kinship matrix concatenated with phenotype DataFrame.")
        ...
        # Define the model
        covariate_cols = k_columns + ['intercept']

        # Fit the model
        model = sm.OLS(herit_df['phenotype'], herit_df[covariate_cols])
        results = model.fit()
